 ...
 * @param runtimeVersion the target runtime version
 */
export async function addMoreHelpfulVMArgs(config: DebugConfiguration, runtimeVersion: number) {
    try {
...
async function shouldShortenIfNecessary(config: DebugConfiguration): Promise<boolean> {
...
function getMaxProcessCommandLineLength(config: DebugConfiguration): number {
...
function getEnvironmentLength(config: DebugConfiguration): number {
