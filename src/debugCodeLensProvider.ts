...
async function constructDebugConfig(
    ...
    if (!debugConfig) {
        ...
        window.showMessage("first: " + (rawConfigs && rawConfigs.length))
        window.showMessage("second: " + await launchJsonExists(workspace))
        if ((rawConfigs && rawConfigs.length) || (await launchJsonExists(workspace))) {
            try {
                ...
                window.showMessage("workspace root: " + Workspace.root)
