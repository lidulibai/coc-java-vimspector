...
function convertToJavaProcess(pid: number, command: string, args: string): IJavaProcess | undefined {
    ...
    if (JAVA_PATTERN.test(simpleName) && args) {
        ...
        if (match && match.length) {
            ...
            const commandPath =
                path.basename(binPath) === 'bin' ? path.join(path.basename(path.dirname(binPath)), 'bin', exeName) : exeName;
            return {
                pid,
                command: commandPath,
                args,
...
export async function pickJavaProcess(): Promise<IJavaProcess | undefined> {
    ...
    try {
        await getProcesses((pid: number, _ppid: number, command: string, args: string) => {
    ...
    items.forEach((value) => itemsString.push(value.detail));

    const pick = await window.showQuickpick(itemsString, 'Pick Java process to attach to');

    if (pick) {
        return items[pick].process;
    }
