...
async function promptMainClass(
    ...
    if (res.length === 0) {
        ...
        window.showErrorMessage(
            `Cannot find a class with the main method${workspaceFolder ? " in the folder '" + workspaceFolder.name + "'" : ''
            }.`
        );
    }
...
async function persistLaunchConfig(
    ...
    if (targetIndex >= 0) {
        window.showMessage("--update")
        rawConfigs[targetIndex] = newConfig;
        launchConfigurations.update('configurations', rawConfigs);
    } else {
        window.showMessage("--update 2")
        launchConfigurations.update('configurations', newConfig);
    }
}
